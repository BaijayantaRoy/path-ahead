...
Before trusting it, the site's single per-school "cut-off" figure was spot-checked against four schools' live MOE SchoolFinder pages, picked for different profiles: Admiralty Secondary (an SAP-adjacent school), Assumption English School (a girls' school with no PG1, and a flat 22 across all five years on the site — the case most likely to be a scraping artifact), Outram Secondary (a dramatic single-year swing, 22 in 2024 to 17 in 2025), and Crescent Girls' School (an otherwise-average school). All four matched MOE's own current figure exactly, and in each case the number turned out to be that school's **PG3 upper bound** — the historic single "cut-off point" quoted before Singapore split posting into PG1/PG2/PG3 bands. Earlier years (2021—2024) could not be checked the same way, because MOE keeps no archive of past years to check them against.

139 of the pack's 147 schools matched a site row by normalized name; the 8 that did not are exactly the specialised-admission schools with no PSLE-score cut-off at all (School of the Arts, NUS High, Singapore Sports School, and the rest of that set) — a consistency check on the matching, not a gap in it. 2 site rows (Fajar Secondary, Teck Whye Secondary) had no match in the pack, apparently closed or merged schools.

Unlike the local overlay, this citation is **tracked and shipped in every build** (`packs/singapore/cutoff-trend-public.yaml`) rather than gitignored, because it republishes a third party's own already-public compilation rather than reproducing MOE's own SchoolFinder page (see `docs/LOCAL_DATA.md`'s new "Third-party public trend citation" section for the full reasoning). `engine/loader.py:_apply_public_cutoff_trend` validates every school id against the pack and computes mean/median/direction per school (mirroring `_cutoff_trend`'s approach for the MOE-sourced bands, but over a single scalar per year); `tools/build_pack.py` ships the source's own attribution alongside it. Every school card with a match renders this as its own boxed table (`publicCutoffTrendTable()` in `web/src/app.js`), always carrying the source's name, a link back to it, and an inline disclaimer that it is community-compiled, not an MOE publication — shown alongside, never instead of, the official current-year SchoolFinder link above it.
