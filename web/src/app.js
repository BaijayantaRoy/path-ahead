...
function paintEnjoy(){
  ...
  for(const s of subs){
    ...
    host.append(el("button",{type:"button",class:"chip","aria-pressed":String(pressed),
      text:s.name, onclick:e=>toggleChip(e.currentTarget,P.enjoyed_subjects,s.code)}));
  }
}
function buildProfileUI(){
  ...
  seg($("#asmSeg"),ASM,v=>P.assessment_style=v);
  seg($("#teamSeg"),TEAM,v=>P.teamwork=v);
...
function renderFilters(){
  ...
  const sel=(id,label,value,opts,onchange)=>el("div",{class:"field"},[
    el("label",{for:id,text:label}),
    el("select",{id,onchange},[el("option",{value:"",text:"Any"}),
      ...opts.map(([v,tx])=>el("option",{value:v,text:tx,...(v===value?{selected:"selected"}:{})}))])]);

  const shown=pool.filter(matchesFilters).length;
  host.replaceChildren(
    el("div",{class:"field"},[
      el("label",{for:"courseSearch",text:"Search"}),
...
function buildNav(track){
  ...
  const link=r=>{
    const a=el("a",{href:r.hash,"data-route":r.id,title:r.label});
...
function renderCoursePage(id){
  ...
  mk.push(el("div",{class:"field field-max"},[
    el("label",{for:"czCourse",text:"Fees shown for"}),
    el("select",{id:"czCourse",onchange:e=>{P.citizenship=e.target.value;
  ...
  out.push(courseSection(9,"Sources",[
    el("ul",{class:"plain"},[...ids].map(sid=>{const s=sourceById(sid); if(!s) return null;
      return el("li",{},[el("strong",{text:s.name||s.id}),el("span",{text:" — "+(s.publisher||"")}),
        s.url?el("span",{},[el("span",{text:" · "}),el("a",{href:s.url,rel:"noopener",target:"_blank",text:"source"})]):null,
        s.retrieved?el("div",{class:"cite",text:"retrieved "+s.retrieved+(s.licence?" · "+s.licence:"")}):null,
      ].filter(Boolean));}).filter(Boolean)),
    el("div",{class:"actions"},[
      o.url?el("a",{class:"btn",href:o.url,rel:"noopener",target:"_blank",text:"The institution's own page"}):null,
      el("a",{class:"btn",href:"#/uni/"+encodeURIComponent(o.institution_short),text:"More from "+o.institution_short}),
  ...
  box.replaceChildren(el("section",{class:"card"},[
    ...
    el("ul",{class:"plain"},p.sources.map(s=>el("li",{},[
      ...
      s.url?el("span",{},[el("span",{text:" · "}),
        el("a",{href:s.url,rel:"noopener noreferrer",target:"_blank",text:"source"})]):null,
      el("div",{class:"cite"},[
  ...
  box.replaceChildren(
    el("section",{class:"card"},[
      ...
      el("ul",{class:"plain"},ROUTES.filter(r=>!r.hidden&&!r.tab).map(r=>
        el("li",{},[el("a",{href:r.hash,text:r.label})]))),
    ]),
    limitsCard(),
  );
}
  ...
  box.replaceChildren(
    el("section",{class:"card"},[
      ...
      el("div",{class:"doors",id:"homeDoors"}, DOORS.map(dr=>{
        ...
        return ready
          ? el("a",{class:"door",href:dr.href,"data-door":dr.stage},kids)
          : el("div",{class:"door unbuilt","data-door":dr.stage,"aria-disabled":"true"},kids);
      })),
    ]),
...
const PS = {score:null, prefs:{postal_code:null, student_sex:null, gender:null, want_sap:null,
  ...
  // is kept separate from `prefs` only because postal code/km and reach
  // work differently under the hood (a chosen band vs. a tri-state match) --
  // both are filters in exactly the same sense.
  filters:{maxKm:null, reachOnly:false}};
      ...
      el("div",{class:"field field-max"},[
        ...
        el("input",{type:"number",id:"psleScore",min:"4",max:"32",inputmode:"numeric",
          ...
          oninput:e=>{ const v=parseInt(e.target.value,10);
            ...
            paintPsleAnswer();
            // The school shortlist's reach filter reads PS.score too (see
            // familyPostingGroups()); without this, entering a score after
            // already scrolling to the shortlist would leave that filter
            // showing "enter your score first" until something else
...
function renderSchoolPrefs(){
  ...
  host.replaceChildren(
        ...
        el("button",{type:"button",class:"ghost",text:"Clear all preferences",
          onclick:()=>{
            PS.prefs = {postal_code:null, student_sex:null, gender:null, want_sap:null,
              want_ip:null, want_autonomous:null, want_gifted:null, school_types:[]};
            PS.filters = {maxKm:null, reachOnly:false};
            renderSchoolPrefs(); renderSchoolShortlist();
          }}),
      ]) : null,
    ...
    /* SAFEGUARDS.md 5.1 forbids ranking schools by cut-off point, so this
       ...
       See engine/school_fit.py:within_reach for the margin and the
       reasoning. */
    (() => {
      ...
      const canUse = PS.score!=null && !!spec;
      return el("div",{class:"field"},[
        el("label",{text:"Only show schools within reach of your PSLE score"}),
        el("div",{class:"seg",role:"group","aria-label":"Reach filter"},[
          el("button",{type:"button","aria-pressed":String(!PS.filters.reachOnly),text:"Show all schools",
            disabled: !canUse,
            onclick:()=>{ PS.filters.reachOnly=false; renderSchoolShortlist(); }}),
          el("button",{type:"button","aria-pressed":String(!!PS.filters.reachOnly),text:"Only schools in reach",
            disabled: !canUse,
            onclick:()=>{ PS.filters.reachOnly=true; renderSchoolShortlist(); }}),
        ]),
        el("p",{class:"hint",text: canUse
          ? "Uses the cut-off figures from your own local copy, plus a margin, since MOE's own "+
            "figures say cut-off points \"can fluctuate by a few points year-on-year.\" These "+
            "figures are yours, not PathAhead's: it has not verified them, and you should check "+
            "anything that matters against the school's own SchoolFinder page, linked on every "+
            "card. A school with no cut-off in your copy still shows, marked as unable to be "+
            "judged rather than hidden."
          : "Enter your child's PSLE score above on this page first — this filter compares it to "+
            "the cut-off figures in your local copy."}),
      ]);
    })(),
  );
  paintPostalStatus();
...
/** Which Posting Group(s) the family's own PSLE score has opened, or null
    if there is no score yet / the pack has no posting_groups spec. Computed
    once per render rather than once per school -- see schoolReach(). */
function familyPostingGroups(){
  if(PS.score==null) return null;
  const spec = psleTransition()?.rule_params?.posting_groups;
  if(!spec) return null;
  return postingGroupFor(spec, PS.score, null).groups || [];
}

/** true / false / null for one school -- see engine/school_fit.py's
    within_reach docstring for what each means. null (never treated as a
    "no") when there is no PSLE score yet, no cutoff published for this
    school, or the score fell outside the Posting Group table entirely. */
function schoolReach(school, familyGroups){
  if(PS.score==null || familyGroups==null) return null;
  return withinReach(school, PS.score, familyGroups);
}
...
function renderSchoolShortlist(){
  ...
  const familyGroups = familyPostingGroups();
  const maxKm = PS.filters.maxKm;
  const reachOnly = PS.filters.reachOnly;
  ...
  const rows = allRows.filter(([school,m])=>{
    ...
    if(reachOnly && schoolReach(school, familyGroups)===false){ hiddenByReach++; return false; }
    return true;
  });
  ...
  if(!rows.length){
    host.replaceChildren(el("p",{class:"hint",id:"schoolNoPrefs",text:
      ...
      "distance, or turning off the reach filter, and PathAhead will show what it hid."}));
    return;
  }
  ...
  for(const [school,m] of rows.slice(0,shown)) list.append(schoolCard(school,m,schoolReach(school,familyGroups)));
  ...
  if(hiddenByReach) bits.push(`${hiddenByReach} outside reach of your PSLE score`);
...
/** Plain-language line for one school's reach status against THIS family's
    own PSLE score -- true/false only; see schoolReach()'s comment for what
    each means. Never says "you missed the cut-off" or any other verdict
...
function reachLine(school, reach){
  if(reach===true) return el("div",{class:"c-sub",text:
    "Last year's Posting Group cut-off suggests this school is within reach of your PSLE score."});
  if(reach===false) return el("div",{class:"c-sub",text:
    "Last year's Posting Group cut-off was better than your PSLE score by more than a few points — "+
    "still allowed as one of your six choices, just less likely on past figures."});
  return null; // no PSLE score yet, or score fell outside the posting table -- nothing extra to say
}
...
function schoolCard(school, match, reach){
  ...
  return el("li",{class:"course","data-school":school.id,style:"grid-template-columns:1fr",
    "data-reach": reach===true?"in-reach":reach===false?"out-of-reach":"unknown"},[
    el("div",{class:"c-top"},[
...
function renderGate(route){
  ...
  box.replaceChildren(el("section",{class:"card"},[
    ...
    el("div",{class:"actions"},[el("a",{class:"btn primary",href:"#/alevel",text:"Go to the questions"})]),
  ]));
}
...
async function boot(){
  ...
  if(elsewhere.length){
    ...
    if(note) note.replaceChildren(el("span",{},[
      ...
      el("a",{href:"#/psle",text:"That is a different system — start here instead"}),
