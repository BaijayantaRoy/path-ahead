...
/* The most recent year's figure from school.cutoff_public_trend (see
   engine/loader.py:_apply_public_cutoff_trend, packs/singapore/cutoff-
   trend-public.yaml) -- a third-party, project-spot-checked citation, not
   MOE's own figure and not a family's own local copy. null when this
   school carries no such citation at all. Mirrors
   engine/school_fit.py:public_trend_current exactly. */
function publicTrendCurrent(school){
  const trend = school.cutoff_public_trend;
  const points = trend && trend.points;
  if(!points || !points.length) return null;
  return points.reduce((best,p)=> p[0]>best[0] ? p : best, points[0])[1];
}

/* Whether this school's most recent PUBLIC historical cut-off falls within
   [lo, hi] inclusive -- the plain "browse schools near a score" filter a
   family types directly (e.g. "AL 13, show me 12 to 14"), which is NOT the
   same question combinedReach()/withinReach() answer above: those need a
   family's own local MOE overlay (cutoff_current) to run at all. This
   works from the data PathAhead actually ships to everyone. A FILTER, not
   a score: never orders schools, and returns null -- never false -- when
   this school carries no public citation to compare against. Mirrors
   engine/school_fit.py:in_score_range exactly. */
function inScoreRange(school, lo, hi){
  const value = publicTrendCurrent(school);
...
const PS = {score:null, prefs:{postal_code:null, student_sex:null, gender:null, want_sap:null,
  ...
  //
  // `scoreRange` is a SEPARATE, simpler filter -- "browse schools whose
  // published-elsewhere historical cut-off sits between these two AL
  // scores" -- built on packs/singapore/cutoff-trend-public.yaml's data,
  // which (unlike `al` above) is present in every build, not just a
  // machine with a private local overlay. See inScoreRange()/
  // publicTrendCurrent() and renderSchoolPrefs' "Browse by historical
  // cut-off" block.
...
function renderSchoolPrefs(){
  ...
  const scoreRangeIsActive = PS.filters.scoreRange.min!=null && PS.filters.scoreRange.max!=null;
  const hasAnyPreference = PS.prefs.postal_code || PS.prefs.student_sex || PS.prefs.gender ||
    ...
    PS.filters.maxKm!=null || alSearchIsActive || scoreRangeIsActive;
  ...
  host.replaceChildren(
    ...
    /* A SEPARATE, simpler filter from the AL-score search below: "show only
       schools whose most recent PUBLIC historical cut-off sits between
       these two AL scores" -- e.g. a family with a real AL 13 wanting to
       browse schools around AL 12 to 14, both a shade above and a shade
       below their own score, rather than a strict "would I get in" check.
       Built on packs/singapore/cutoff-trend-public.yaml (docs/LOCAL_DATA.md),
       which -- unlike the local MOE overlay the AL-score search below
       needs -- ships in every build, so this is usable for the 139 of 147
       schools that citation covers without anyone supplying their own
       copy of anything. See inScoreRange()/publicTrendCurrent(). */
    (() => {
      const hasPublicTrend = (S.pack?.schools||[]).some(s=>s.cutoff_public_trend);
      if(!hasPublicTrend) return null;
      const sr = PS.filters.scoreRange;
      const onField = (field) => (e) => {
        const v = parseInt(e.target.value,10);
        sr[field] = Number.isFinite(v) && v>=4 && v<=32 ? v : null;
        renderSchoolShortlist();
      };
      return el("div",{class:"field",role:"group","aria-label":"Historical cut-off range filter"},[
        el("label",{text:"Browse by historical cut-off (public data)"}),
        el("p",{class:"hint",text:
          "Not the AL-score search below -- this shows schools whose most recently recorded "+
          "cut-off falls in a range you choose, so a family working from a real or expected AL "+
          "score can see nearby schools on both sides of it, not only ones that score would "+
          "reach. Leave either box empty and this filter does nothing."}),
        el("div",{class:"field field-max"},[
          el("label",{for:"scoreRangeMin",text:"From (AL score)",
            title:"PSLE Achievement Level sum: 4 (best) to 32 (worst)."}),
          el("input",{type:"number",id:"scoreRangeMin",min:"4",max:"32",inputmode:"numeric",
            title:"Show schools with a historical cut-off at or above this AL score.",
            value: sr.min==null ? "" : String(sr.min), oninput: onField("min")}),
        ]),
        el("div",{class:"field field-max"},[
          el("label",{for:"scoreRangeMax",text:"To (AL score)",
            title:"PSLE Achievement Level sum: 4 (best) to 32 (worst)."}),
          el("input",{type:"number",id:"scoreRangeMax",min:"4",max:"32",inputmode:"numeric",
            title:"Show schools with a historical cut-off at or below this AL score.",
            value: sr.max==null ? "" : String(sr.max), oninput: onField("max")}),
        ]),
        el("p",{class:"hint",text:
          "Uses SG School Kaki's community-compiled figures (2025, the most recent year on "+
          "file) -- not an MOE publication, and not independently verified for every school "+
          "(see each card's own cut-off history for the disclaimer and a link to the source). "+
          "A school with no figure on file still shows, marked as unable to be judged rather "+
          "than hidden -- that covers the 8 specialised-admission schools with no PSLE cut-off "+
          "at all, and any other school the source itself carries no row for."}),
      ]);
    })(),
...
function renderSchoolShortlist(){
  ...
  const sr = PS.filters.scoreRange;
  const scoreRangeActive = sr.min!=null && sr.max!=null;
  const scoreRangeLo = scoreRangeActive ? Math.min(sr.min, sr.max) : null;
  const scoreRangeHi = scoreRangeActive ? Math.max(sr.min, sr.max) : null;
  let hiddenBySex = 0, hiddenByPrefs = 0, hiddenByKm = 0, hiddenByReach = 0, hiddenByScoreRange = 0;
  const rows = allRows.filter(([school,m])=>{
    ...
    if(scoreRangeActive && inScoreRange(school, scoreRangeLo, scoreRangeHi)===false){ hiddenByScoreRange++; return false; }
    return true;
  });
  ...
  if(!rows.length){
    host.replaceChildren(el("p",{class:"hint",id:"schoolNoPrefs",text:
      ...
      "distance, widening the historical cut-off range, or clearing the AL score search, and "+
      "PathAhead will show what it hid."}));
    return;
  }
  ...
  for(const [school,m] of rows.slice(0,shown))
    list.append(schoolCard(school,m,alSearchReach(school),
      scoreRangeActive ? inScoreRange(school, scoreRangeLo, scoreRangeHi) : null));
  ...
  if(hiddenByScoreRange) bits.push(`${hiddenByScoreRange} outside the historical cut-off range you set`);
...
/** A third-party historical cut-off trend, shown on every school card that
    has one -- independent of whether a local MOE overlay is present,
    unlike cutoffHistoryTable() above. `school.cutoff_public_trend` arrives
    precomputed from engine/loader.py:_apply_public_cutoff_trend
    (packs/singapore/cutoff-trend-public.yaml -- tracked and published,
    unlike packs/singapore/local/cutoff.json, because this is a citation of
    a third party's own already-public compilation, not a republishing of
    MOE's own SchoolFinder figure -- see that file's own header comment).

    Always carries its own attribution and disclaimer inline, and is never
    presented as PathAhead's own figure or as equivalent to the officially-
    linked current-year figure above it. */
function publicCutoffTrendTable(school){
  const trend = school.cutoff_public_trend;
  const source = S.pack && S.pack.cutoff_public_trend_source;
  if(!trend || !source) return null;
  const years = trend.points.map(p=>p[0]).slice().sort((a,b)=>b-a);
  const DIR = {up:"risen (a higher score needed)", down:"eased (a lower score needed)",
               flat:"held steady", single:"—"};
  const oldestYear = years[years.length-1];

  return el("div",{class:"psle-cutoff-history psle-cutoff-history-public"},[
    el("p",{class:"hint",text:`Historical trend, ${oldestYear}–${years[0]} — not from PathAhead or MOE:`}),
    el("table",{class:"cmp"},[
      el("thead",{},[el("tr",{},[el("th",{text:"Year"}),el("th",{text:"PG3 cut-off"})])]),
      el("tbody",{},years.map(y=>{
        const pt = trend.points.find(p=>p[0]===y);
        return el("tr",{},[el("td",{text:String(y)}),el("td",{text: pt ? String(pt[1]) : "—"})]);
      })),
      el("tfoot",{},[
        el("tr",{},[el("th",{text:"Mean"}),el("td",{text:String(trend.mean)})]),
        el("tr",{},[el("th",{text:"Median"}),el("td",{text:String(trend.median)})]),
      ]),
    ]),
    el("p",{class:"hint",text:
      (years.length>1 ? `Has ${DIR[trend.direction]} since ${oldestYear}. ` : "")+source.disclaimer}),
    el("p",{class:"hint"},[
      el("span",{text:"Source: "}),
      el("a",{href:source.url,target:"_blank",rel:"noopener noreferrer",text:source.name}),
    ]),
  ]);
}
...
function cutoffRangeLine(school){
  ...
  return el("div",{},[
    el("div",{class:"c-sub","data-cutoff":"","data-cutoff-origin":"linked"},[
...
/** Plain-language line for whether this school's most recent PUBLIC
    historical cut-off (packs/singapore/cutoff-trend-public.yaml) falls in
    the range a family typed into "Browse by historical cut-off" -- a
    DIFFERENT question from reachLine() above, which needs a local MOE
    overlay and answers "would this score likely get in". This just states
    whether the school's own figure sits inside the chosen window,
    factually, with no verdict language (SAFEGUARDS.md 5.3). `inRange` is
    null when the filter is not active OR this school has no public figure
    to compare -- either way there is nothing extra to say here, since the
    no-figure case is already covered once, unconditionally, by
    cutoffRangeLine()/publicCutoffTrendTable() above. */
function scoreRangeLine(inRange){
  if(inRange===true) return el("div",{class:"c-sub",text:
    "Its most recent public historical cut-off falls inside the range you're browsing."});
  // inRange===false never actually reaches this function -- renderSchoolShortlist() hides those
  // schools before schoolCard() is ever called for them, the same way reachLine()'s own
  // "out-of-reach" never does. Kept as an explicit branch anyway, for the same reason: a future
  // change to that filter should not silently start showing an unlabelled card.
  if(inRange===false) return el("div",{class:"c-sub",text:
    "Its most recent public historical cut-off falls outside the range you're browsing."});
  return null;
}

function schoolCard(school, match, reach, inRange){
  const meta = el("div",{class:"c-meta"},[
